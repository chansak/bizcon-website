var dialogBox = {
    ...
    show: function(url, title, winWidth, winHeight, bgColor, dragAble) {
        ...
        //}
        $(document.body).hide().append($(this.background));
        $(document.body).append($(this.dialog)).fadeIn(500);
        this.body.src = this.getURL(this.body.src);
        $(this.background).click(function () { dialogBox.close(); });
    ...
    close: function(returnValue) {
        ...
        this.isShowed = false;
        $(this.background).fadeOut(500, function () { $(this.background).remove(); });
        $(this.dialog).fadeOut(500, function () { $(this.dialog).remove(); });
    },
    ...
    showHtml: function(html, title, settings) {
    if (dialogBox.isShowed) dialogBox.close();
        this.init(null,title, settings.width, settings.height, settings.bgColor, settings.drag);
        this.isShowed = true;
    ...
    SelectedSize: function (popupType)
    {
